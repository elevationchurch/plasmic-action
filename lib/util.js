...
function mkPackageManagerCmds(cwd) {
    ...
    const usePnpm = fs_1.existsSync(path_1.default.join(cwd, "pnpm-lock.yaml")) ||
        fs_1.existsSync(path_1.default.join(".", "pnpm-lock.yaml"));
    if (useYarn) {
...
function mkPrBranchName(targetBranch) {
    return `plasmic-sync/${nanoid_1.nanoid(8)}/${targetBranch}`;
}
